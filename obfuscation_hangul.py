...
def combine_syllable(cho, jung, jong):
    ...
    if cho < 0 or cho >= len(CHOSUNG_LIST):
        return   # 유효하지 않은 초성은 물음표로 처리
    if jung < 0 or jung >= len(JUNGSUNG_LIST):
        return   # 유효하지 않은 중성은 물음표로 처리
    if jong < 0 or jong >= len(JONGSUNG_LIST):
        return   # 유효하지 않은 종성은 물음표로 처리
...
train_output = train_output.drop_duplicates().reset_index(drop = True)
...
train_df = pd.concat([train , aug_train]).reset_index(drop = True) # 기존 train 데이터와 concat하기
train_df = train_df.sample(frac=1).reset_index(drop=True) # 순서 섞기

train_df.to_csv('./data/aug_train.csv') # 증강된 데이터 저장